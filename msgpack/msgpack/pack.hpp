...
public:
	...
	template <typename T>
	packer<Stream>& pack_service_array(std::string strElementClass, const std::vector<T>& v)
	{
		pack_array(v.size() + 2); 
		pack_true();
		pack(strElementClass);

		for(typename std::vector<T>::const_iterator it(v.begin()), it_end(v.end()); it != it_end; ++it) {
			pack(*it);
		}
		return *this; 
	}
	...
	packer<Stream>& pack_service_map(std::string strKeyClass, std::string strValueClass, const std::map<K, V >& v)
	{
		if (v.empty()) {
			pack_array(1); 
            pack_false();
		} else {
			pack_array(v.size() * 2 + 3); 
			pack_true();
			pack(strKeyClass);
			pack(strValueClass);
			
			for(typename std::map<K, V>::const_iterator it(v.begin()), it_end(v.end()); it != it_end; ++it) {
				pack(it->first);
				pack(it->second);
			}			
		}
